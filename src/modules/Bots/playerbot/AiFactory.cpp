...
int AiFactory::GetPlayerSpecTab(Player* bot)
{
    ...
    // For most classes the DBC tabpage value works, but some classes have
    // multiple talent trees sharing the same tabpage (e.g. Arcane and Fire
    // both have tabpage 0), so we assign unique class-specific tab values.
    switch (bot->getClass())
    {
        case CLASS_MAGE:
            // Arcane(81)=tab 0, Fire(41)=tab 1, Frost(61)=tab 2
            if (bestId == 41) return 1;     // Fire
            if (bestId == 61) return 2;     // Frost
