...
bool AttackAction::isUseful()
{
    ...
    if (sPlayerbotAIConfig.tankDelaySeconds == 0 &&
        sPlayerbotAIConfig.tankThreatPct == 0.0f)
        return true;
    ...
    if (sPlayerbotAIConfig.tankDelaySeconds > 0)
    {
        ...
        if (getMSTimeDiff(m_tankEngageTime, getMSTime()) <
            sPlayerbotAIConfig.tankDelaySeconds * 1000)
            return false;
    }
    ...
    if (sPlayerbotAIConfig.tankThreatPct > 0.0f)
    {
        float threshold = target->GetMaxHealth() *
            sPlayerbotAIConfig.tankThreatPct / 100.0f;
