...
bool MovementAction::FollowOnTransport(Unit* target, Player* master)
{
    ...
    if (isApproaching && approachTimedOut)
    {
        ...
        bot->NearTeleportTo(loc.x, loc.y, loc.z, loc.orientation);
        bot->SetTransport(transport);
        transport->AddPassenger(bot);
        bot->m_movementInfo.SetTransportData(
            transport->GetObjectGuid(),
            bot->GetPositionX() - transport->GetPositionX(),
            bot->GetPositionY() - transport->GetPositionY(),
            bot->GetPositionZ() - transport->GetPositionZ(),
            bot->GetOrientation() - transport->GetOrientation(),
            getMSTime()
        );
...
bool MovementAction::FollowOffTransport(Unit* target, Player* master)
{
    ...
    if(!transport || transport != botTransport) // master has left the transport
    {
        ...
        uint32 currentTime = time(0);
        if (transportBoardingDelayTime == 0)
        {
            // First detection - wait for master to walk away from the edge
            transportBoardingDelayTime = currentTime;
            return false;
        }

        if (currentTime - transportBoardingDelayTime < 2 && distanceToMaster <= sPlayerbotAIConfig.sightDistance)
        {
            // Still waiting for master to move away from the boarding edge
            return false;
        }
